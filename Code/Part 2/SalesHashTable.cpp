...
category::category(string name , double average_pr , double nb_kg , int penalty ) : category_name{name},
    average_price{average_pr}, nb_kg_sold{nb_kg}, severity_penalty{penalty} 
    {
       ...
       if (average_pr < 0 || nb_kg < 0)
            {
                 throw out_of_range("the number of kilograms and the average price can NOT take negative values ");
            }

      if (penalty < 0 || penalty > 3)
{
            throw out_of_range("the penalty severity need to be between 1 to 3 ");
        }
    }
    ...
    Sales::Sales(long id , category *name , int m , int y ,  double w , double el )
        : farmer_id{id}, month{m}, year{y}, water{w} , electricity{el}
     {
         ...
         if(farmer_id < 0 || farmer_id > 200000){
            throw out_of_range("this ID doesn't exist in our dataset ");
        }
...
Sales SaleHashTable ::getSales(long key) // it returns the element inside the object in the array cell where the key is the farmer's id
{
