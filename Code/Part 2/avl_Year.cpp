...
avl_Year ::avl_Year() : root{nullptr}
{
...
// copy constructor
avl_Year ::avl_Year(const avl_Year &rhs) : root{nullptr}
{
    root = clone(rhs.root);
    wilayas = rhs.wilayas;
    cities = rhs.cities;
...
// Move constructor
avl_Year::avl_Year(avl_Year &&rhs) : root{rhs.root}
{
    rhs.root = nullptr;
...
// copy assignment
avl_Year &avl_Year ::operator=(const avl_Year &rhs)
{
    avl_Year copy = rhs;
    std::swap(*this, copy);
    return *this;
...
// Move assignment
avl_Year &avl_Year::operator=(avl_Year &&rhs)
{
    std::swap(root, rhs.root);
    avl_Year copy = std::move(rhs);
    wilayas = rhs.wilayas;
    areas = rhs.areas;
    cities = rhs.cities;
    farmers = rhs.farmers;
...
// insert a sales object to the avl_Year
void avl_Year::insert(const Sales &sl)
{
    ...
    // the yearSales object (hold) into our avl_Year
...
//
void avl_Year::reportSales(int year, long id) const
{
...
void avl_Year ::reportCountry(int year) const
{
    if (year < 1990 || year > 2022)
        throw out_of_range("invalid year");
...
//
void avl_Year::reportSales(int year, int month, long id) const
{
    if (year < 1990 || year > 2022)
        throw out_of_range("invalid year");
...
void avl_Year::reportPenalties(int year, int month, long id) const
{
    if (year < 1990 || year > 2022)
        throw out_of_range("invalid year");
...
void avl_Year::reportPenalties(int year, long id) const
{
     if (year < 1990 || year > 2022)
        throw out_of_range("invalid year");
...
void avl_Year ::reportArea(int year, int areaID) const
{
...
void avl_Year::reportCity(int year, int cityId) const
{
    if (year < 1990 || year > 2022)
        throw out_of_range("invalid year");
...
void avl_Year::reportWilaya(int year, int wilayaID) const
{
    if (year < 1990 || year > 2022)
        throw out_of_range("invalid year");
...
void avl_Year::penalty_farmers_city(int year, int cityId)
{
    if (year < 1990 || year > 2022)
        throw out_of_range("invalid year");
...
void avl_Year::penalty_farmers_Wilaya(int year, int wilayaID)
{
    if (year < 1990 || year > 2022)
        throw out_of_range("invalid year");
...
void avl_Year::reportFarmer(int farmerId) const
{
    ...
    cout << "the fqrmer area   :" << farmers[farmerId].farmer_area << endl;
...
//  ' IDs
void avl_Year::fill_areas(const avl_Year::Farmer &obj)
{
...
void avl_Year::init(string file_path)
{
    ifstream file;
    file.open(file_path);
    if (!file)
...
void avl_Year::read_farmers(string path)
{
    ifstream file;
    file.open(path);
    if (!file)
 ...
 */
void avl_Year::insert(const yearSales &x, BinaryNode *&t)
{
    if (t == nullptr)
    {
 ...
 * Set the new root of the subtree.
 */
void avl_Year::insert(yearSales &&x, BinaryNode *&t)
{
    if (t == nullptr)
 ...
 * Set the new root of the subtree.
 */
void avl_Year::remove(const yearSales &x, BinaryNode *&t)
{
    if (t == nullptr)
...
bool avl_Year::contains(const yearSales &x, BinaryNode *t) const
{
    if (t == nullptr)
        return false;
    else if (x < t->element)
...
void avl_Year::makeEmpty(BinaryNode *&t)
{
    if (t != nullptr)
    {
        makeEmpty(t->left);
...
void avl_Year ::balance(BinaryNode *&t)
{
    if (t == nullptr)
        return;
...
void avl_Year ::rotateWithLeftChild(BinaryNode *&k2)
{
...
// right right imbalance
void avl_Year ::rotateWithRightChild(BinaryNode *&k1)
{
...
// Left right imbalance
void avl_Year ::doubleWithLeftChild(BinaryNode *&k3)
{
...
// right left imbalance
void avl_Year ::doubleWithRightChild(BinaryNode *&k1)
{
    rotateWithLeftChild(k1->right);
    rotateWithRightChild(k1);
}

vector<float> avl_Year::findSales(long id, int month, int year, bool& worked){
