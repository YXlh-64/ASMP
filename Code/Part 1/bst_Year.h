...
class bst_Year
{
  public:
            ...
            : farmer_Id{i}, farmer_area{area}, farmer_SSN{SSN}, farmer_fullName{name},
              farmer_gender{gender}, farmer_age{age}, nb_helpers{helpers} {
                if(farmer_Id < 0 || farmer_Id > 200000){
                    throw out_of_range("the id is not valid ");
                  }
              };
    };
     ...
     * Copy assignment
     */
    bst_Year & operator=( const bst_Year & );
    
        
    /**
     * Move assignment
     */
    bst_Year & operator=( bst_Year &&  );
    ...
    //  ' IDs
    void fill_areas(const bst_Year::Farmer &);
